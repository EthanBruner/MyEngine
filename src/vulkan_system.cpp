...
VulkanSystem::VulkanSystem(Window& window, const char* vertexPath, const char* fragmentPath) : window{ window } {
    createInstance();
    setupDebugMessenger();
    createSurface();
    pickPhysicalDevice();
...
void VulkanSystem::createGraphicsPipeline(const std::string vertexPath, const std::string fragmentPath) {
...
void VulkanSystem::loadModel() {
    ...
    if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, "C:\\Users\\Ethan\\Documents\\C++\\MyEngine\\assests\\viking_room.obj")) {
        throw std::runtime_error(warn + err);
    }

    std::unordered_map<VertexObject, uint32_t> uniqueVertices{};
